...
public class OddBetResultHandler implements BetResultHandler {

    @Override
    public List<Bet> handle(List<Bet> allBets, int winingNumber) {
        return allBets.stream().filter(bet -> bet.getSlot().equalsIgnoreCase("ODD"))
                .map(b -> Bet.builder()
                        .player(b.getPlayer())
                        .slot(b.getSlot())
                        .amount(b.getAmount())
                        .payout(b.getAmount() * Payout.ODD.getAmount())
